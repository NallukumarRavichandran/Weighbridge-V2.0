...
if ($qChoice && $qChoice->num_rows > 0) {
    $activePrintFormat = $qChoice->fetch_assoc()['format_key'];
}

// Initial Local Formats for Dropdown: Anchor default_slip at the top
$localSavedFormats = [
    ['format_key' => 'default_slip', 'name' => 'Default Print Slip (Local)']
];
$resLocalFmt = $conn->query("SELECT `format_key`, `name` FROM `print_format_settings` WHERE `company_name` != 'ACTIVE_CHOICE' AND `format_key` != '' AND `format_key` != 'default_slip'");
if ($resLocalFmt && $resLocalFmt->num_rows > 0) {
    while ($rf = $resLocalFmt->fetch_assoc()) {
        $localSavedFormats[] = $rf;
    }
}

if (count($localSavedFormats) === 1) {
    $localSavedFormats = array_merge($localSavedFormats, [
        ['format_key' => 'format_standard_a4_portrait', 'name' => 'Standard A4 Portrait Slip'],
        ['format_key' => 'format_dual_landscape_cctv',    'name' => 'Dual-Copy Top-Bottom A4 (Stacked 2-in-1 with CCTV)'],
        ['format_key' => 'format_single_dotmatrix',       'name' => 'Single Copy Continuous (TVS Dot-Matrix)'],
        ['format_key' => 'format_triplicate_landscape',   'name' => 'Triplicate Copy Landscape A4 Slip']
    ]);
}
...
function openPrintSettings() {
    ...
    syncTemplatesCatalog(null, true);
}
...
function updateSelectedTemplateStatus() {
    ...
    if (found && found.is_cached) {
        btn.innerText = "Downloaded";
        btn.title = "Already cached on disk (Click to re-download)";
    } else {
