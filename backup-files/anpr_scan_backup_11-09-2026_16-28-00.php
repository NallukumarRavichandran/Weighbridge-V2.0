...
function extractVehicleNumber($fullText, $lines = []) {
    ...
    // 2 state letters + 1-2 RTO digits + 1-3 series letters + strictly 4 number digits
    $pattern = '/\b([A-Z]{2})\s*[-.]?\s*([0-9]{1,2})\s*[-.]?\s*([A-Z]{1,3})\s*[-.]?\s*([0-9]{4})\b/i';
    ...
    // Secondary scan on stripped clean text (strictly requires 2 state letters + 1-2 RTO digits + series + 4 number digits)
    $sanitized = preg_replace('/[^A-Z0-9]/', '', strtoupper($fullText));
    if (preg_match('/([A-Z]{2})([0-9]{1,2})([A-Z]{1,3})([0-9]{4})/', $sanitized, $m)) {
        $rto = str_pad($m[2], 2, '0', STR_PAD_LEFT);
