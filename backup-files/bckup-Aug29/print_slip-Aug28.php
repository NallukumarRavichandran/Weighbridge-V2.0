...
// Intelligent field mapper for dynamic metadata
$material_name = '';
$party_name    = '';
$vessel_name   = '';
$vt_no         = '';
...
/* ============================================================
   RESOLVE 1ST AND 2ND WEIGHMENT PHOTOS FROM DATABASE
   ============================================================ */
$print_images = [];

$firstImg  = (!empty($row['first_image_path']) && file_exists(__DIR__ . '/' . $row['first_image_path'])) ? $row['first_image_path'] : null;
$secondImg = (!empty($row['second_image_path']) && file_exists(__DIR__ . '/' . $row['second_image_path'])) ? $row['second_image_path'] : null;

if ($firstImg && $secondImg) {
    // Both 1st and 2nd weighment photos exist from database
    $print_images[] = ['title' => '1ST WEIGHMENT PHOTO', 'path' => $firstImg];
    $print_images[] = ['title' => '2ND WEIGHMENT PHOTO', 'path' => $secondImg];
} elseif ($firstImg) {
    // Only 1st weighment photo exists
    $print_images[] = ['title' => '1ST WEIGHMENT PHOTO', 'path' => $firstImg];
} elseif ($secondImg) {
    // Only 2nd weighment photo exists
    $print_images[] = ['title' => '2ND WEIGHMENT PHOTO', 'path' => $secondImg];
}
?>
<!DOCTYPE html>
...
/* ============================================================
   PHOTOS SECTION (1ST & 2ND WEIGHMENT PROPORTIONAL GRID)
   ============================================================ */
.slip-photos-section {
    margin: 8px 0;
    width: 100%;
...
<div class="slip-container">
    ...
    <!-- 2-COLUMN VEHICLE & MATERIAL/PARTY DETAILS -->
    ...
    <!-- CAPTURED PHOTOS FROM 1ST & 2ND WEIGHMENT -->
    <?php if (!empty($print_images)): ?>
    <div class="divider-dashed"></div>
    <div class="slip-photos-section">
        <div class="photos-grid <?= (count($print_images) === 1) ? 'single-photo' : '' ?>">
            <?php foreach ($print_images as $img): ?>
            <div class="photo-card">
                <div class="photo-title"><?= htmlspecialchars($img['title']) ?></div>
                <img src="<?= htmlspecialchars($img['path']) ?>" alt="<?= htmlspecialchars($img['title']) ?>">
            </div>
            <?php endforeach; ?>
        </div>
    </div>
