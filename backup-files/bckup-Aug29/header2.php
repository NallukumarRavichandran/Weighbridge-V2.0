...
$conn->query("CREATE TABLE IF NOT EXISTS `print_format_settings` (
    `id` INT AUTO_INCREMENT PRIMARY KEY,
    `company_name` VARCHAR(255) NOT NULL UNIQUE DEFAULT 'ALL',
...
.enterprise-dialog-box {
    ...
    color: #1e293b;
    width: 95%;
    max-width: 980px; /* Spacious width for large screens */
    margin: 15px auto;
    border-radius: 8px;
...
<!-- ============================================================
     PRINT SETTINGS, FIELD VISIBILITY & LOGO DIALOG (LARGE VISIBILITY)
     ============================================================ -->
            ...
            <div class="dialog-body">
                ...
                <div class="win-groupbox">
                    <div class="win-group-title">
                        ...
                        <button type="button" class="btn-win-sync" onclick="syncTemplatesFromCloud()">
                            🔄 Sync from Cloud
                        </button>
                    </div>
                    <div>
                        <select id="local_print_layout_select" name="selected_print_layout" style="width:100%;padding:9px 12px;border:1.5px solid #cbd5e1;border-radius:5px;font-size:14px;font-weight:700;color:#0f172a;">
                            ...
                            <option value="format_dual_landscape_cctv" <?= ($activePrintFormat==='format_dual_landscape_cctv')?'selected':'' ?>>2. Dual-Copy Landscape A4 (2-in-1 Balajee Steels 4 CCTV)</option>
...
<script>
/* ============================================================
   PRINT SETTINGS MODAL CONTROLS & CLOUD SYNC
   ============================================================ */
    ...
    fetch("https://cloud.online-weighing.in/weighment_printslip.php?action=get_active_formats")
        ...
        .then(data => {
            ...
            if (data.status === "success" && data.formats && data.formats.length > 0) {
                ...
                alert("Synchronized " + data.formats.length + " active layout formats from Cloud!");
            }
        })
        .catch(err => {
            selectEl.disabled = false;
            alert("Cloud sync notice: Using local cached formats.");
        });
}
