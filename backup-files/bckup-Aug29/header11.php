...
/* ---------------- SERVER-SIDE CLOUD TEMPLATE DOWNLOADER ENGINE ---------------- */
if (isset($_GET['action']) && $_GET['action'] === 'download_from_cloud') {
    ...
    if ($response === false || empty($response)) {
        echo json_encode([
            'status' => 'error', 
            'message' => 'Could not connect to cloud server: ' . ($curlErr ?: 'Empty response')
        ]);
        exit;
    }

    $data = json_decode($response, true);
    if (!$data || !isset($data['formats']) || !is_array($data['formats'])) {
        echo json_encode([
            'status' => 'error', 
            'message' => 'Invalid JSON payload received from cloud: ' . substr(strip_tags($response), 0, 150)
        ]);
        exit;
    }

    // 1. Ensure local templates folder exists
    $tplDir = __DIR__ . '/templates';
    if (!is_dir($tplDir)) {
        @mkdir($tplDir, 0777, true);
    }

    // 2. Ensure local database table & columns exist
    ...
    // 3. Save each template into local database and write to templates/ folder
    ...
    foreach ($data['formats'] as $fmt) {
        ...
        if (!empty($fKey)) {
            ...
            // Write template as executable .php file
            if (!empty($fCode)) {
    ...
    echo json_encode([
        'status'  => 'success',
        'message' => "Successfully downloaded and updated {$savedCount} templates locally.",
...
/* ============================================================
   ENTERPRISE DIALOG STYLING (LARGE VISIBILITY)
   ============================================================ */
#cameraSettingsModal,
#printSettingsModal {
    display: none;
...
<!-- ============================================================
     PRINT SETTINGS, FIELD VISIBILITY & LOGO DIALOG
     ============================================================ -->
...
<script>
/* ============================================================
   PRINT SETTINGS MODAL CONTROLS & SERVER-SIDE CLOUD DOWNLOADER
   ============================================================ */
...
function downloadTemplatesFromCloud(btn) {
    ...
    // Calls local PHP cURL downloader (100% immune to browser CORS / SSL issues)
    fetch("header.php?action=download_from_cloud")
        .then(res => res.json())
        .then(data => {
            ...
            if (data.status === "success" && data.formats && data.formats.length > 0) {
                ...
                alert("✓ Successfully downloaded " + data.formats.length + " live template formats from Cloud into your local machine!");
            } else {
                alert("Download Notice: " + (data.message || "Failed to parse cloud formats."));
            }
        })
        .catch(err => {
            if (btn) btn.innerText = originalText;
            if (selectEl) selectEl.disabled = false;
            alert("Download Notice: Could not connect to local server.");
        });
}
