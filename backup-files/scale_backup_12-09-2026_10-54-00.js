...
function getSavedScaleProfile() {
    try {
        // Ensure 2400 Baud is the active profile (overriding any old cached 9600 baud)
        if (localStorage.getItem("scale_v2_profile_set") !== "2400") {
            localStorage.setItem("scale_working_profile", JSON.stringify(UNIVERSAL_SCALE_PROFILES[0]));
            localStorage.setItem("scale_v2_profile_set", "2400");
...
async function connectScaleDirect() {
    ...
    try {
        ...
        const baudSelect = document.getElementById("scaleBaudSelect");
        let profile;
        if (baudSelect) {
            const idx = parseInt(baudSelect.value, 10);
            profile = UNIVERSAL_SCALE_PROFILES[idx] || UNIVERSAL_SCALE_PROFILES[0];
            saveWorkingScaleProfile(profile);
        } else {
            profile = getSavedScaleProfile();
        }
...
function openScaleDialog() {
    ...
    // Inject Baud Rate selector if not present
    let baudBox = document.getElementById("scaleBaudContainer");
    if (!baudBox) {
        const inner = dlg.querySelector("div");
        if (inner) {
            baudBox = document.createElement("div");
            baudBox.id = "scaleBaudContainer";
            baudBox.style.margin = "12px 0";
            baudBox.style.textAlign = "left";

            const curProfile = getSavedScaleProfile();
            let optionsHtml = UNIVERSAL_SCALE_PROFILES.map((p, idx) => {
                const sel = (p.baudRate === curProfile.baudRate && p.parity === (curProfile.parity || "none")) ? "selected" : "";
                return `<option value="${idx}" ${sel}>${p.name}</option>`;
            }).join("");

            baudBox.innerHTML = `
                <label style="display:block;font-size:12px;font-weight:bold;color:#333;margin-bottom:4px;">Indicator Baud Rate:</label>
                <select id="scaleBaudSelect" style="width:100%;padding:6px;font-size:13px;border:1px solid #ccc;border-radius:4px;background:#fff;color:#000;">
                    ${optionsHtml}
                </select>
            `;
            ...
            if (btn) {
                inner.insertBefore(baudBox, btn);
            } else {
                inner.appendChild(baudBox);
            }
        }
    } else {
        const curProfile = getSavedScaleProfile();
        const selEl = document.getElementById("scaleBaudSelect");
        if (selEl) {
            UNIVERSAL_SCALE_PROFILES.forEach((p, idx) => {
                if (p.baudRate === curProfile.baudRate && p.parity === (curProfile.parity || "none")) {
                    selEl.value = idx;
                }
            });
        }
