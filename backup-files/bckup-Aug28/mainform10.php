...
if (isset($_GET['camera'])) 
{
    ...
    if (!empty($camUrl)) {
        // Map MJPEG live stream endpoint to single-frame snapshot endpoint for fast grab
...
/* ============================================================
   ENTERPRISE 2X2 CAMERA CCTV GRID (BALANCED 205PX PROPORTIONS)
   ============================================================ */
.camera-grid-section {
    margin-top: 15px;
    width: 100%;
...
.camera-card {
    ...
    height: 205px; /* Balanced breadth and height */
    border-radius: 4px;
...
function validateVehicleNumber() {
    ...
    // Accepts both 1-letter (TN99Z9999) and 2-letter (TN99ZZ9999) series
    const pattern = /^[A-Z]{2}[0-9]{2}[A-Z]{1,2}[0-9]{4}$/;

    if (!pattern.test(vehicle)) {
        alert("Vehicle Number must be in the format TN01AB1234 or TN01A1234");
...
/* MANUAL ENTER WEIGHT (FROM LEFT PANEL INPUT) */
function applyManualWeight() {
    const manualInput = document.getElementById("manual_weight_input");
    let manualVal = manualInput ? manualInput.value.trim() : "";

    if (!manualVal || isNaN(parseFloat(manualVal)) || parseFloat(manualVal) <= 0) {
        alert("Please enter a valid positive weight in the box!");
        if (manualInput) manualInput.focus();
        return;
    }

    let weightNum = parseFloat(manualVal);
    let formattedWeight = (weightNum % 1 !== 0) ? weightNum.toFixed(2) : weightNum;

    const liveEl = document.getElementById("live_weight");
    if (liveEl) liveEl.innerText = formattedWeight;

    window.weightFrozen = true;
    frozenWeight = formattedWeight;

    if (secondMode) {
        const target = document.getElementById(secondType + "_weight");
        if (target) target.value = formattedWeight;
        setDateTime(secondType);
    } else {
        let gtSelect = document.querySelector("[name='gt_type']");
        let gt = gtSelect ? gtSelect.value : "G";
...
function resetWeighment() {
    ...
    const mInput = document.getElementById("manual_weight_input");
...
/* ============================================================
   AUTO-RECONNECT ON RELOAD (300MS WINDOWS SETTLE DELAY)
   ============================================================ */
...
window.addEventListener("load", function () {
    setTimeout(function () {
        if (typeof autoReconnect === "function") {
            console.log("Auto-reconnecting scale to COM4 on reload...");
            autoReconnect();
        }
    }, 300);
});
...
/* ============================================================
   DYNAMIC CCTV CAMERA GRID CONTROLLER (ADAPTIVE 1, 2, 3, 4 CAMS)
   ============================================================ */
...
function applyCameraGridVisibility() {
    ...
    // Adaptive Heights matching 205px proportions
    if (count === 1) {
...
<!-- ENTER WEIGHT MANUAL ENTRY ROW -->
<div class="field-group">
<label>ENTER WEIGHT</label>
<div style="display:inline-flex; gap:8px; vertical-align:middle;">
    <input type="number" step="any" id="manual_weight_input" placeholder="ENTER WEIGHT" style="width:180px;" onkeydown="if(event.key==='Enter'){ event.preventDefault(); applyManualWeight(); }">
    <button type="button" onclick="applyManualWeight()" style="width:auto; padding:6px 14px; background:#f59e0b; color:#fff; border:none; border-radius:4px; font-weight:bold; cursor:pointer;">ENTER</button>
</div>
</div>
...
<!-- ============================================================
     DYNAMIC RESPONSIVE CCTV GRID (1, 2, 3, 4 CAMS ADAPTIVE)
     ============================================================ -->
