...
<?php
/* VEHICLES FOR SECOND WEIGHMENT (PARAMETRIC QUERIES) */
$vehicleData = [];
...
if ($stmt_v) {
    $stmt_v->bind_param("i", $company_id);
    $stmt_v->execute();
    $q = $stmt_v->get_result();
    ...
    while ($r = $q->fetch_assoc()) {
        ...
        ");
        if ($stmt_d) {
            $slipId = (int)$r['slip_no'];
            $stmt_d->bind_param("i", $slipId);
            $stmt_d->execute();
            $dq = $stmt_d->get_result();
...
/* DYNAMIC FIELDS (PARAMETRIC QUERY) */
$fields = [];
$stmt_f = $conn->prepare("
    SELECT *
...
if ($stmt_f) {
    $stmt_f->bind_param("i", $company_id);
    $stmt_f->execute();
...
<script>
// Validates that the vehicle number is entered
...
/* SECOND WEIGHMENT MODE: POPULATES DETAILED SLIP OPTIONS */
...
/* RECORD WEIGHT (FROM SCALE STREAM) */
...
<input
    ...
    oninput="this.value=this.value.toUpperCase(); handleVehicleTyping(this.value);"
    placeholder="ENTER VEHICLE NO">
<select id="vehicle_select" style="display:none;width:280px;"></select>
</div>
