...
if ($gross_wt == 0 || $tare_wt == 0) {
    ...
    $checkVehicle->bind_param("si", $vehicle_no, $company_id);
    $checkVehicle->execute();
    $pending = $checkVehicle->get_result()->fetch_assoc();	
...
function captureCamera($slip_no, $type)
{
    ...
    // Load active camera config from local config.php if present
    ...
    if (!empty($camUrl)) {
        ...
        curl_setopt($ch, CURLOPT_TIMEOUT, 2); // 2s timeout prevents saving delay
