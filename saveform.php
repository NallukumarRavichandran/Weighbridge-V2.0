...
// FLOAT CASTING TO PRESERVE DECIMALS (e.g. 10.2, 7.5)
$gross_wt   = (float)($_POST['gross_weight'] ?? 0);
$tare_wt    = (float)($_POST['tare_weight'] ?? 0);
...
/* ============================================================
   FIRST WEIGHMENT (ONLY ONE WEIGHT AVAILABLE - NO VEHICLE LOCK)
   ============================================================ */
if ($gross_wt == 0 || $tare_wt == 0) {
    ...
    if (!$stmt->execute()) {
        die("Error saving first weighment: " . $stmt->error);
    }
...
/* ============================================================
   SECOND WEIGHMENT (FINAL SAVE & APACS DISPATCH)
   ============================================================ */
...
try {
    ...
    /* ---- INSERT FINAL RECORD WITH DUPLICATE PROTECTION ---- */
    $stmt = $conn->prepare("
	...
	/* ---- CREATE APACS PENDING ENTRY (SAFE UPSERT PREVENTING DUPLICATE KEY ERROR) ---- */
    $apacs = $conn->prepare("
        INSERT INTO apacs_upload_log
        (slip_no, status)
        VALUES (?, 'PENDING')
        ON DUPLICATE KEY UPDATE `status` = 'PENDING', `request_data` = NULL, `response_data` = NULL
    ");
    ...
    $apacsFields = [];
    $q = $conn->query("
        SELECT wf.field_name, wf.field_label,
               wfv.field_value
    ...
    while ($r = $q->fetch_assoc()) {
        ...
        if (!empty($r['field_label'])) {
            $apacsFields[$r['field_label']] = $r['field_value'];
        }
    }
    ...
    $payload = array(
        ...
        "weighDate"       => $weighDate,
        "weighTime"       => ($gross_date >= $tare_date) ? $gross_time : $tare_time,
        "vehicleNumber"   => $vehicle_no,
        "movementType"    => strtolower($apacsNorm['movementtype'] ?? ""),
        "cargo"           => $apacsNorm['cargo'] ?? $apacsNorm['materialname'] ?? "",
        "clientName"      => $apacsNorm['clientname'] ?? $apacsNorm['partyname'] ?? "",
        "grossWeight"     => $gross_wt,
        "tareWeight"      => $tare_wt,
        "netWeight"       => $net_wt,
        "weightUnit"      => strtolower($apacsNorm['weightunit'] ?? "kg")
    );
...
/* ============================================================
   CAPTURE LIVE CAMERA FRAME AT EXACT SAVE MOMENT
   ============================================================ */
function captureCamera($slip_no, $type)
{
    ...
    if (file_exists(__DIR__ . '/config.php')) {
        ...
        if (!empty($wbConfig['cameras'][1]['url'])) {
            $camUrl  = $wbConfig['cameras'][1]['url'];
            $camUser = $wbConfig['cameras'][1]['username'] ?? 'admin';
            $camPass = $wbConfig['cameras'][1]['password'] ?? '';
        }
    }
    ...
    if (!empty($camUrl)) {
        ...
        if ($img && ($httpCode == 200 || $httpCode == 0)) {
...
/* ============================================================
   FUNCTION : SAVE FIRST-WEIGHMENT DYNAMIC FIELDS (WITH AUTO-CLEANUP)
   ============================================================ */
function saveDynamicFields($conn, $company_id, $slip_no)
{
    // Clean any prior duplicate entries for this slip
    $conn->query("DELETE FROM weighment_field_values WHERE weighment_id = $slip_no");
