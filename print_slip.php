...
if ($qChoice && $qChoice->num_rows > 0) {
    ...
    if (!empty($rowChoice['format_key'])) {
        $activeFormat = trim($rowChoice['format_key']);
    }
} else {
    $qFmt = $conn->query("SELECT `format_key` FROM `print_format_settings` WHERE `company_name` = 'ALL' LIMIT 1");
    if ($qFmt && $qFmt->num_rows > 0) {
        $rowFmt = $qFmt->fetch_assoc();
        if (!empty($rowFmt['format_key'])) {
            $activeFormat = trim($rowFmt['format_key']);
        }
    }
}

// Sanitize format key for local file safety
$activeFormat = preg_replace('/[^a-zA-Z0-9_-]/', '', $activeFormat);
if (empty($activeFormat)) {
    $activeFormat = 'default_slip';
}

// Check if static .php template exists in local templates/ folder
$tplFile = __DIR__ . '/templates/' . $activeFormat . '.php';

// Check if local database has dynamic template_code fallback
$dbTemplateCode = '';
if ($conn) {
    $qCode = $conn->query("SELECT `template_code` FROM `print_format_settings` WHERE `format_key` = '" . $conn->real_escape_string($activeFormat) . "' AND `template_code` IS NOT NULL AND `template_code` != '' LIMIT 1");
